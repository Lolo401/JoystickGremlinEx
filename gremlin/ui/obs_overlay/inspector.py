...
class ColorButton(QtWidgets.QPushButton):
    ...
    def _pick(self):
        ...
        if chosen.isValid():
            ...
            if previous.alpha() == 0 and chosen.alpha() == 0:
                chosen.setAlpha(255)
...
class OverlayInspector(QtWidgets.QWidget):
    ...
    def rebuild(self):
        ...
        try:
            ...
            bar = self._scroll.verticalScrollBar() if getattr(self, "_scroll", None) is not None else None
            if bar is not None:
                bar.setValue(0)
        except Exception:
    ...
    def _build_canvas(self):
        ...
        if onscreen:
            ...
            chroma = ColorButton(canvas.get("chroma_color") or "#00FF00")
    ...
    def _build_widget(self, item: dict):
        ...
        if widget_type == "button":
            ...
            self._style_color(look, item, "border", "Off border")
            self._style_color(look, item, "border_on", "On border")
            ...
            self._style_bool(look, item, "invert_display", "Invert display")
            ...
            if widget_type == "hat":
                ...
                self._style_color(look, item, "crosshair", "Crosshair")
            else:
                self._indicator_shape(look, item)
                self._style_bool(look, item, "show_dot_shadow", "Dot shadow")
                self._style_bool(look, item, "show_dot_crosshair", "Lines through dot")
            ...
            self._style_bool(look, item, "show_axis_labels", "Axis labels")
            for key, title in (("axis_label_n", "North"), ("axis_label_s", "South"), ("axis_label_e", "East"), ("axis_label_w", "West")):
                ...
                look.addRow(title, edit)
            self._style_font(look, item, prefix="axis_label_")
            self._style_color(look, item, "axis_label_font_color", "Axis label color")
        ...
        if widget_type not in ("button", "shape", "panel", "label", "axis_radio", "image"):
            self._style_color(look, item, "border", "Border")
        if widget_type != "label":
            self._style_float(look, item, "border_width", "Border width", 0, 20)
            if widget_type not in NO_CORNER_RADIUS_TYPES and widget_type not in ("shape", "panel", "image"):
                self._style_float(look, item, "corner_radius", "Corner radius", 0, 200)
        self._style_float(look, item, "opacity", "Opacity", 0.05, 1.0, 0.05)
        if widget_type not in ("label", "panel", "shape", "image"):
            ...
            look.addRow("Deadzone display", dead)
            if not self._multi:
    ...
    def _build_mixed(self, items: list[dict]):
        ...
        if types <= {"button"}:
            ...
            self._style_color(look, item, "border", "Border")
        self._style_float(look, item, "border_width", "Border width", 0, 20)
        if not types <= set(NO_CORNER_RADIUS_TYPES):
            self._style_float(look, item, "corner_radius", "Corner radius", 0, 200)
        self._style_float(look, item, "opacity", "Opacity", 0.05, 1.0, 0.05)
        ...
        if types <= invert_types:
            self._style_bool(look, item, "invert_display", "Invert")
        if not types.intersection({"label", "panel", "shape", "image"}):
            dead = QtWidgets.QDoubleSpinBox()
            dead.setRange(0.0, 0.9)
            dead.setSingleStep(0.01)
            dead.setValue(float(item["style"].get("deadzone") or 0))
    ...
    def _slider_int(self, form, title: str, value: int, lo: int, hi: int, on_change):
        value = int(value)
        lo, hi = int(lo), int(hi)
        if hi < lo:
            lo, hi = hi, lo
    ...
    def _crosshair_appearance(self, form, item: dict):
    ...
    def _build_toggle_binding(self):
        ...
        self._build_channel(
            ...
            show_clear=True,
            show_invert=False,
            extra_hint="While the profile is running, press this button to show or hide the overlay.",
        )
